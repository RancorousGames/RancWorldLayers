...
public:
	...
	bool TestFindNearestPointWithValue() const
	{
		...
		bFound = Subsystem->FindNearestPointWithValue(TestLayerName, FVector2D(10.0f, 10.0f), 10.0f, FLinearColor(1.0f, 0.0f, 0.0f, 0.0f), FoundLocation);
		Res &= Test->TestTrue("Should find a point near (10,10)", bFound);
		Res &= Test->TestEqual("Found point X should be 10.5", (float)FoundLocation.X, 10.5f, 0.1f);
		Res &= Test->TestEqual("Found point Y should be 10.5", (float)FoundLocation.Y, 10.5f, 0.1f);

		// Test 2: Search near (50,50)
		bFound = Subsystem->FindNearestPointWithValue(TestLayerName, FVector2D(50.0f, 50.0f), 10.0f, FLinearColor(1.0f, 0.0f, 0.0f, 0.0f), FoundLocation);
		Res &= Test->TestTrue("Should find a point near (50,50)", bFound);
		Res &= Test->TestEqual("Found point X should be 50.5", (float)FoundLocation.X, 50.5f, 0.1f);
		Res &= Test->TestEqual("Found point Y should be 50.5", (float)FoundLocation.Y, 50.5f, 0.1f);

		// Test 3: Search for a non-existent value
		bFound = Subsystem->FindNearestPointWithValue(TestLayerName, FVector2D(50.0f, 50.0f), 10.0f, FLinearColor(0.5f, 0.0f, 0.0f, 0.0f), FoundLocation);
		Res &= Test->TestFalse("Should not find a non-existent value", bFound);

		// Test 4: Search outside MaxSearchRadius
		bFound = Subsystem->FindNearestPointWithValue(TestLayerName, FVector2D(1.0f, 1.0f), 5.0f, FLinearColor(1.0f, 0.0f, 0.0f, 0.0f), FoundLocation);
