...
public:
	...
	bool TestSubsystemInitialization() const
	{
		FDebugTestResult Res = true;
		WorldDataLayersTestContext Context(Test);
		// Subsystem is already tested for null in the context constructor
		return Res;
	}
	...
	bool TestDataLayerValueManipulation() const
	{
		...
		Res &= Test->TestEqual("Value at TestLocation2 should match TestValue2", OutValue2.R, TestValue2.R, KINDA_SMALL_NUMBER);
...
bool FRancWorldLayersTest::RunTest(const FString& Parameters)
{
	...
	bResult &= Scenarios.TestSubsystemInitialization();
